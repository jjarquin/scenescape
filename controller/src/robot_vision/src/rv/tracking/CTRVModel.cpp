...
void CTRVModel::stateConversionFunction(const cv::Mat &x_k, const cv::Mat &u_k, const cv::Mat &v_k, cv::Mat &x_kplus1)
{
  ...
  // double nextYaw = velocityYaw + yawRate * deltaT; // Velocity yaw for the next time iteration

  // double cosNextYaw = cos(nextYaw);
  // double sinNextYaw = sin(nextYaw);

  const double WT = yawRate * deltaT;

  if (fabs(WT) >= 1e-4)
  {
    // Polychronopoulos, Aris et al. (2007).
    ...
    const double SW = sinWT / WT;
    const double CW = (1 - cosWT) / WT;

    x_kplus1.at<double>(0, 0) = x + deltaT * (SW * vx - CW * vy);
    x_kplus1.at<double>(1, 0) = y + deltaT * (CW * vx + SW * vy);
    x_kplus1.at<double>(3, 0) = vx * cosWT - vy * sinWT;
    x_kplus1.at<double>(4, 0) = vx * sinWT + vy * cosWT;
  }
  else
  {
...
cv::Mat CTRVModel::processNoiseCovariance(double processNoise, double deltaT, int type)
{
  return cv::Mat::eye(17, 17, type) * processNoise;
}

cv::Mat CTRVModel::measurementNoiseCovariance(double measurementNoise, double deltaT, int type)
{
